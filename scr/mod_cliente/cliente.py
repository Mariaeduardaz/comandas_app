...
def insert():
    try:
        ...
        payload = {'id_cliente': id_clinete, 'nome': nome, 'cpf': cpf, 'telefone': telefone, 'comprar_fiado': comprar_fiado, 'dia_fiado': dia_fiado, 'senha': senha }
...
def formEditCliente():
    try:
        ...
        response = requests.get(ENDPOINT_CLIENTE + id_clinete, headers=HEADERS_API)
        result = response.json()
        if (response.status_code != 200):
            raise Exception(result[0])
...
def edit():
    try:
        ...
        payload = {'id_clinete': id_clinete, 'nome': nome, 'cpf': cpf, 'telefone': telefone, 'comprar_fiado': comprar_fiado, 'dia_fiado': dia_fiado, 'senha': senha }
        # executa o verbo PUT da API e armazena seu retorno
        response = requests.put(ENDPOINT_CLIENTE + id_clinete, headers=HEADERS_API, json=payload)
        result = response.json()
...
def delete():
    try:
        # dados enviados via FORM
        id_produto = request.form['id_clinete']
        # executa o verbo DELETE da API e armazena seu retorno
        response = requests.delete(ENDPOINT_CLIENTE + id_produto, headers=HEADERS_API)
        result = response.json()
        if (response.status_code != 200 or result[1] != 200):
            raise Exception(result[0])
